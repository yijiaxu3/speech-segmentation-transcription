...
class VoiceActivityDetector():
    ...
    def __init__(self, wave_input_filename):
        ...
        self.speech_start_band = 300 #300
        self.speech_end_band = 2000 #3000

# Infants show a range of 250 to 650 Hz, and in some cases go over 1000 Hz.  \
#A 10-year-old boy or girl might have a fundamental frequency around 400 Hz.           
    ...
    def _calculate_amplitude(self, audio_data):
        data_ampl = np.abs(np.fft.fft(audio_data)) # same number as audio_data 
        data_ampl = data_ampl[1:]
        return data_ampl
        
    def _calculate_energy(self, data):
        data_amplitude = self._calculate_amplitude(data) #aplitudes at different frequency for that data series signal 
        data_energy = data_amplitude ** 2 # sqaure amplitude is the energy 
        return data_energy
    ...
    def _calculate_normalized_energy(self, data):
        ...
        # contains the frequency bin centers in cycles per unit of the sample spacing
        # same number as data 
        data_energy = self._calculate_energy(data)
        # same number as data 
        #d ata_energy = self._znormalize_energy(data_energy) #znorm brings worse results
        energy_freq = self._connect_energy_with_frequencies(data_freq, data_energy)
        # each frequency after fft corresponds to a squared amplitude value / energy 
        return energy_freq
    ...
    def detect_speech(self):
        ...
        while (sample_start < (len(data) - sample_window)):
            ...
            energy_freq = self._calculate_normalized_energy(data_window) # each f of the fft of that data signal has corresponding amplitude^2 or energy 
            sum_voice_energy = self._sum_energy_in_band(energy_freq, start_band, end_band) # sum of energies correponding to the valid frequency bands 
            sum_full_energy = sum(energy_freq.values()) # sum of all frequencies' energies 

            # speech_ratio = sum_voice_energy/sum_full_energy
            # speech_ratio = speech_ratio>self.speech_energy_threshold
            
            # speech_ratio = sum_full_energy >= 1.7e11 #2e11

            speech_ratio = sum_voice_energy >=0.3e9 #>= 0.55e10->1e9
            
            detected_windows = np.append(detected_windows,[sample_start, speech_ratio])
            sample_start += sample_window - sample_overlap
        ...
        # [20ms window start sample number, 0/1]
